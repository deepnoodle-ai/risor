...
// TestVMWrapperErrorRecovery tests that the VM can recover from errors
// and continue executing new code without repeating the error.
func TestVMWrapperErrorRecovery(t *testing.T) {
	vm, err := NewVM()
	assert.Nil(t, err)

	// Execute some valid code first
	result, err := vm.Eval(context.Background(), "let x = 5")
	assert.Nil(t, err)
